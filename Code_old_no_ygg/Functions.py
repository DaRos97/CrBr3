...
def compute_magnetization(Phi,pars,args_minimization):
    ...
    for sss in range(args_minimization['rand_m']):
        if min_E<1e8:
            np.save(name_phi(pars,False),result)
        E = []  #list of energies for the while loop
        #Initial condition
        ans = sss if sss==0 else 1
        fs = ((sss-1)//2)/2 if (sss>0 and sss<65) else random.random()
        fa = ((sss-1)%2)/2 if (sss>0 and sss<65) else random.random()
        ...
        while True:
            ...
            if step > args_minimization['maxiter']:
                print("Exeeded number of steps and final energy was ",E[0])
...
def initial_point(Phi,pars,fs,fa,ans):
    ...
    gamma,alpha,beta = pars
    pts_array,pts_gamma,grid,pts_per_fit,learn_rate_0,A_M = inputs.args_general
#    initial_ansatz = ['t-s_custom','t-s_pert','t-s2_custom','t-a_custom','constant']
    ...
    if sol=='t-s_pert':      #ansatz for alpha,beta<<1
        ...
        print(sol)
    elif sol=='t-s2_custom':
        phi_s = (1-np.sign(Phi))*np.pi
        phi_a = (np.sign(Phi)-1)*np.pi/2
        print(sol)
    elif sol=='t-a_custom':
        phi_s = (1-np.sign(Phi))*np.pi/2
        phi_a = (1-np.sign(Phi))*np.pi/2
        print(sol)
    return [phi_s, phi_a]
